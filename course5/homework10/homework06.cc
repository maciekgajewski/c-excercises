...
void test_basics()
{
	...
	T str3;
	str3 = "Haleluja"; // user defined assignment with char*
...
void test_operators()
{
	...
	T str(test_str);
	unsigned int str_len_before = str.length();
	T str_copy = str;
	std::cout << "Testing move. Length before = "<< str_len_before << std::endl;

	T str2 = std::move(str);
	unsigned int str_len_after = str.length();

	std::cout << "Length after move: "<< str_len_after << std::endl;
	assert(str_len_after == 0);
	assert(str_len_before == str2.length());
	std::cout << "str2     == " << str2.c_str() << str2.length() << std::endl;
	std::cout << "test_str == " << test_str << std::strlen(test_str) << std::endl;
	assert(str2 == test_str);
	assert(str2 == str_copy);
	...
	std::cout << "Iterating over string using iterators: ";
	for (IT i = str2.begin(); i != str2.end(); ++i)
	{
		std::cout << *i;
	}
	std::cout << std::endl;
...
int main()
{
	...
	test_operators<MyString, mystring::iterator>();
	std::cout << "End of tests" << std::endl;

	std::cout << "Start of tests for std::string"<< std::endl;
	test_basics<std::string>();
	test_operators<std::string, std::string::iterator>();
	std::cout << "End of tests" << std::endl;
