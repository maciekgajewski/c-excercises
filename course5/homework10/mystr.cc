...
{
	//printf("In operator == for char*. Length = %d %d\n", std::strlen(mStr.get()), std::strlen(str));
...
const mystring::iterator MyString::begin()
{
	return mystring::iterator(*this, 0);
}

const mystring::iterator MyString::end()
{
	return mystring::iterator(*this, this->length());
}

iterator::iterator(const MyString& str, int pos)
{
	m_str = str;
	m_pos = pos;
}

mystring::iterator iterator::operator++()
{
	m_pos++;
	return *this;
}
