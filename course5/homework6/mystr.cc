...
MyString:: MyString(const MyString&& str)  // move constructor
{
	Reset_mStr(str.length(), str.c_str());
}

MyString& MyString::operator = (const MyString&& str)  // move assignment
{
	Reset_mStr(str.length(), str.c_str());
	return *this;
}
...
MyString&  MyString::operator+= (const MyString& str) // concatenation assignment
{
	...
	std::memset(new_str.get(), 0, new_size + 1);
